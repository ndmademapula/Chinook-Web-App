...
if slt_br == br_options[2]:
    ...
    with monthyear:
        ...
        req_3_monthyear = run_query(query)
...
if slt_br == br_options[4]:
    ...
    req_5_time = run_query(query_time)
    query_year = Query.query_5_year
    req_5_year = run_query(query_year)
...
if slt_br == br_options[5]:
    ...
    req_6_time = run_query(query_time)
    query_year = Query.query_6_year
    req_6_year = run_query(query_year)
    selected_emp = st.multiselect(
        label='Select Employee', 
...
if slt_br == br_options[6]:
    ...
    with st.expander('Number of Genre in store'):
        query = '''select distinct * from star.DimGenre'''
        req = run_query(query)
        st.dataframe(req)
    day, month, quarter, year = st.tabs(['Day','Month','Quarter','Year'])
    with day:
        query = Query.query_7_day
        req_7 = run_query(query)
...
if slt_br == br_options[8]:
    ...
    req_9_last = run_query(query_last)
    req_9_avg = run_query(query_avg)
